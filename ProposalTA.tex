...
\begin{center}
    ...
    % 1. Top Padding (Keep this fixed if you want the title slightly down)
    \vspace*{1cm} 

    % 2. Title Block
    {\Large\bfseries SISTEM RANGKUMAN OTOMATIS BERBASIS AI UNTUK REDUKSI INFORMATION OVERLOAD DALAM KOMUNITAS FINANSIAL TELEGRAM BERJENJANG} \\[0.5cm]
    {\large\bfseries (Studi Kasus: Grup Telegram Michael Yeoh dan Richy Matthew Ashari)}
    ...
    % 5. Logo (Ensure the path is correct)
    \begin{figure}[h]
        \centering
        \includegraphics[width=0.25\textwidth]{image/ganesha.jpg} % Increased size slightly
    \end{figure}

    % Dynamic spacing 4
    \vfill

    % 6. Footer / Institution
    \textbf{
    ...
    % Assuming you have datetime package loaded, or just hardcode the year
    {\large 2025} 
    }
    
    % Bottom padding to ensure it doesn't touch the edge
    \vspace*{1cm}
